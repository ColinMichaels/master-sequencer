...
export const discoverRenderResults = async (outputRoot) => {
  ...
  await Promise.all(files.filter((filePath) => path.basename(filePath).includes(".part.")).map((filePath) => rm(filePath, { force: true })));
  for (const manifestPath of files.filter((filePath) => filePath.endsWith("-render-manifest.json"))) {
    try {
      ...
      results.push({
        ...
        createdAt: manifest.createdAt || fileStat.mtime.toISOString(),
        recovered: true,
      });
    } catch {
