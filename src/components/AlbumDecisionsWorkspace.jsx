  ...
  return (
    <main className="decisions-workspace">
      ...
      <section className="decision-module readiness-inspector" aria-labelledby="readiness-title">
        ...
        <div className="readiness-table" role="table"><div className="readiness-head" role="row"><strong>Track</strong>{gateLabels.map(([, label]) => <span key={label}>{label}</span>)}</div>{readiness.map((row) => <div className="readiness-row" role="row" key={row.trackId}><strong>{row.title}</strong>{gateLabels.map(([key, label, shortLabel]) => key === "humanApproval" ? <label key={key} className={row.gates[key] ? "is-ready" : ""} data-gate-label={shortLabel}><input type="checkbox" checked={row.gates[key]} onChange={(event) => onAlbumChange((draft) => { draft.tracks.find((track) => track.id === row.trackId).humanApproved = event.target.checked; })} /><span className="sr-only">{label} for {row.title}</span>{row.gates[key] ? <CheckIcon /> : <WarningIcon />}</label> : <span key={key} className={row.gates[key] ? "is-ready" : ""} data-gate-label={shortLabel} aria-label={`${label}: ${row.gates[key] ? "ready" : "not ready"}`}>{row.gates[key] ? <CheckIcon /> : <WarningIcon />}</span>)}</div>)}</div>
      </section>
