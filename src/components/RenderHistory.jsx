  ...
  return (
    <section className="render-history" aria-labelledby="render-history-title">
      ...
      {renders.length > 0 && <>
        ...
        {comparison.length > 0 ? <div className="manifest-diff" role="table">{comparison.map((row) => <div role="row" key={`${row.field}-${row.left}-${row.right}`}><strong>{row.field}</strong><span>{row.left}</span><span>{row.right}</span></div>)}</div> : leftId && rightId && <p className="render-history-empty"><WarningIcon /> {compared ? "No documented differences were found." : "Choose two renders and compare their documentation."}</p>}
      </>}
    </section>
  );
}
