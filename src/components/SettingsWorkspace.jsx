    ...
    <main className="settings-workspace">
      ...
      <div className="settings-columns">
        <section className="settings-section">
          <div className="settings-section-heading"><h3>{hostedDemo ? "Demo Audio" : "Audio Paths"}</h3><button type="button" className="text-button" disabled={hostedDemo || scanning} title={hostedDemo ? "The hosted tester uses protected Album 1 streams" : "Rescan every configured source"} onClick={onRescan}><RefreshIcon /> {scanning ? "Scanning…" : hostedDemo ? "Album 1" : "Rescan All"}</button></div>
          {hostedDemo && <p className="hosted-capability-note"><strong>Safe hosted boundary:</strong> this site stores project decisions in this browser and plays the released Album 1 through protected streams. It cannot see device paths, local masters, Finder, or FFmpeg exports.</p>}
          {!hostedDemo && <>
        ...
        <section className="settings-section">
          ...
          <p className="settings-note"><strong>{hostedDemo ? "Browser project export:" : "Portable checksum bundle:"}</strong> {hostedDemo ? "downloads project decisions only. Protected Album 1 audio is not included." : "exports project JSON plus SHA-256 records for currently indexed sources. It contains no media bytes and does not copy audio."}</p>
          {importError && <p className="render-error" role="alert">{importError}</p>}
          <dl className="data-locations">{hostedDemo ? <><div><dt>Open project</dt><dd>Browser local storage</dd></div><div><dt>Saved projects</dt><dd>This browser only</dd></div><div><dt>Demo audio</dt><dd>Protected Dreadnauts stream</dd></div><div><dt>Device paths</dt><dd>Never requested</dd></div></> : <><div><dt>Open project</dt><dd>data/sequencer-state.json</dd></div><div><dt>Saved projects</dt><dd>data/projects/</dd></div><div><dt>Audio metadata cache</dt><dd>data/audio-index-cache.json</dd></div><div><dt>File and folder paths</dt><dd>config/sequencer.local.json</dd></div></>}</dl>
