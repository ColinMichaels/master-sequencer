    ...
    <main className="settings-workspace">
      ...
      <div className="settings-columns">
        <section className="settings-section">
          <div className="settings-section-heading"><h3>{hostedDemo ? "Demo Audio" : "Audio Paths"}</h3><button type="button" className="text-button" disabled={hostedDemo || scanning} title={hostedDemo ? "The hosted tester uses official Album 1 previews" : "Rescan every configured source"} onClick={onRescan}><RefreshIcon /> {scanning ? "Scanning…" : hostedDemo ? "Album 1" : "Rescan All"}</button></div>
          {hostedDemo && <p className="hosted-capability-note"><strong>Safe hosted boundary:</strong> this site stores project decisions in this browser and plays official 30-second previews from the released Album 1. It cannot see device paths, local masters, Finder, or FFmpeg exports.</p>}
          {!hostedDemo && <>
        ...
        <section className="settings-section">
          ...
          <p className="settings-note"><strong>{hostedDemo ? "Browser project export:" : "Portable checksum bundle:"}</strong> {hostedDemo ? "downloads project decisions only. Album 1 preview audio is not included." : "exports project JSON plus SHA-256 records for currently indexed sources. It contains no media bytes and does not copy audio."}</p>
